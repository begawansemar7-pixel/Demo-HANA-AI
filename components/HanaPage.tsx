...
export const useHanaChatLogic = (storageKey: string | null, isComponentActive: boolean) => {
    ...
    const isThinking = hanaState === HanaState.THINKING;
    
    // The button should only be truly disabled when thinking or if the session is over.
    // This allows clicking it to interrupt speech or to stop listening.
    const isInteractionDisabled = isThinking || showPaymentWall || isSessionEnded;
    
    // The text input should be disabled when thinking, speaking, OR listening.
    const isInputDisabled = isThinking || isSpeaking || isListening || showPaymentWall || isSessionEnded;
...
const HanaPage: React.FC = () => {
  ...
  return (
    <>
      ...
      <ListeningModal isOpen={isListening} onStop={handleMicClick} />
    </>
  );
};
